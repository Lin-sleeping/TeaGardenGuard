...
public class IotDeviceController extends BaseController {
    ...
    @Autowired
    private MqttPushClient mqttPushClient; // 你的MQTT推送Bean
    ...
    public AjaxResult bindDevice(@RequestBody IotDevice iotDevice) {
        ...
        return AjaxResult
                .success(iotDeviceService.bindDevice(userId, username, deviceNum, deviceName, categoryId, remark));
    }
    ...
    @ApiOperation(value = "切换设备在线状态", notes = "切换设备enable字段")
    @PreAuthorize("@ss.hasPermi('system:device:edit')")
    @Log(title = "设备", businessType = BusinessType.UPDATE)
    @PostMapping("/changeEnable")
    public AjaxResult changeEnable(@RequestBody Map<String, Object> params) {
        Long deviceId = Long.valueOf(params.get("deviceId").toString());
        int enable = Integer.parseInt(params.get("enable").toString());
        int result = iotDeviceService.updateDeviceEnable(deviceId, enable);

        // 1. 获取设备编号（deviceNum）等信息
        IotDevice device = iotDeviceService.selectIotDeviceById(deviceId);
        if (device != null) {
            String deviceNum = device.getDeviceNum();
            // 2. 构造MQTT主题和消息内容
            String topic = deviceNum;
            String payload = params.get("enable").toString();
            // 3. 发送MQTT消息
            mqttPushClient.publish(1, true, topic, payload);
        }
